...
router.get('/', async (req, res) => {
    try {
        const { limit, offset, search, tag, subcategory_id, category_id } = req.query;
...
router.post('/', async (req, res) => {
    try {
        const { subcategory_id, title, arabic, transliteration, translation, reference, tags, } = req.body;
        if (!title)
            return res.status(400).json({ error: 'Missing title' });
        const result = await db.run(`INSERT INTO duas (subcategory_id, title, arabic, transliteration, translation, reference, tags) 
       VALUES (?, ?, ?, ?, ?, ?, ?)`, subcategory_id || null, title, arabic || null, transliteration || null, translation || null, reference || null, tags || null);
        const inserted = await db.get('SELECT * FROM duas WHERE id = ?', result.lastID);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (!existing)
            return res.status(404).json({ error: 'Dua not found' });
        const { subcategory_id, title, arabic, transliteration, translation, reference, tags, } = req.body;
        await db.run(`UPDATE duas SET subcategory_id = ?, title = ?, arabic = ?, transliteration = ?, translation = ?, reference = ?, tags = ? WHERE id = ?`, subcategory_id || null, title ?? existing.title, arabic ?? existing.arabic, transliteration ?? existing.transliteration, translation ?? existing.translation, reference ?? existing.reference, tags ?? existing.tags, id);
